...
* `object-detection` (Supported model types - yolox, yolov3, yolov5, yolov8)
...
<Tip warning={true}>
The detault model is only supported for `object-detection task. For other tasks, please provide the `model_id / RyzenAIModel` for inference.`
</Tip>

## Using Ryzen models

The [`~ryzenai.pipelines.pipeline`] function accepts Ryzen supported models from the [Hugging Face Hub](https://huggingface.co/models?other=RyzenAI)

Once you have picked an appropriate model, you can create the [`~ryzenai.pipelines.pipeline`] by specifying the model repo:
...
method associated with the `RyzenModelForXXX` class.

For example, here is how you can load the [`~ryzenai.RyzenModelForImageClassification`] class for image classification:
...
Note for a few models, `model_type` amd `image_preprocessor` has to be provided in addition to the model_id for inference.

For example, here is how you can run inference using `yolox`
